...
void
CreateParticleGrid(Vec3 lower, int dimx, int dimy, int dimz, float radius, Vec3 velocity, float invMass, bool rigid,
...
void
CreateParticleSphere(Vec3 center, int dim, float radius, Vec3 velocity, float invMass, bool rigid, float rigidStiffness,
...
void CreateSpring(int i, int j, float stiffness, float give = 0.0f) {
    ...
    g_buffers->springLengths.push_back(
            (1.0f + give) * Length(Vec3(g_buffers->positions[i]) - Vec3(g_buffers->positions[j])));
...
void CreateParticleShape(const Mesh *srcMesh, Vec3 lower, Vec3 scale, float rotation, float spacing, Vec3 velocity,
                         ...
                         Vec3 skinOffset = 0.0f, float skinExpand = 0.0f, Vec4 color = Vec4(0.0f),
...

    {
        ...
        Voxelize((const Vec3 *) &mesh.m_positions[0], mesh.m_positions.size(), (const int *) &mesh.m_indices[0],
                 mesh.m_indices.size(), maxDim, maxDim, maxDim, &voxels[0], meshLower,
            ...
            for (int y = 0; y < maxDim; ++y) {
                for (int z = 0; z < maxDim; ++z) {
                    ...
                    if (voxels[index]) {
                        ...
                        Vec3 position =
                                lower + meshLower + spacing * Vec3(float(x) + 0.5f, float(y) + 0.5f, float(z) + 0.5f) +
                            ...
                            for (int i = x - width; i <= x + width; ++i) {
                                for (int j = y - width; j <= y + width; ++j) {
                                    for (int k = z - width; k <= z + width; ++k) {
                                        ...
                                        if (neighborCell > 0 && neighborCell < int(voxels.size()) &&
                                            voxels[neighborCell] && neighborCell != centerCell) {
...
void CreateParticleShape(const char *filename, Vec3 lower, Vec3 scale, float rotation, float spacing, Vec3 velocity,
                         float invMass, bool rigid, float rigidStiffness, int phase, bool skin, float jitter = 0.005f,
                         Vec3 skinOffset = 0.0f, float skinExpand = 0.0f, Vec4 color = Vec4(0.0f),
                         float springStiffness = 0.0f) {
            ...
            for (int i = startVertex; i < numVertices + startVertex; ++i) {
                ...
                for (int w = 0; w < 4; ++w) {
                    ...
                    if (g_meshSkinIndices[i * 4 + w] > -1) {
                        ...
                        skinPos += (rotation * (g_meshRestPositions[i] - Point3(g_buffers->restPositions[index])) +
                                    Vec3(g_buffers->positions[index])) * weight;
                    }
                }
...
void AddBox(Vec3 halfEdge = Vec3(2.0f), Vec3 center = Vec3(0.0f), Quat quat = Quat(), bool dynamic = false,
            int channels = eNvFlexPhaseShapeChannelMask) {
    // transform
...
void CreateSDF(const Mesh *mesh, uint32_t dim, Vec3 lower, Vec3 upper, float *sdf) {
    if (mesh) {
        ...
        Voxelize((const Vec3 *) &mesh->m_positions[0], mesh->m_positions.size(), (const int *) &mesh->m_indices[0],
                 mesh->m_indices.size(), dim, dim, dim, volume, lower, upper);
...
void AddRandomConvex(int numPlanes, Vec3 position, float minDist, float maxDist, Vec3 axis, float angle) {
    ...
    for (uint32_t j = 0; j < builder.mIndices.size(); j += 3) {
        ...
        Vec3 n = Normalize(
                Cross(builder.mVertices[b] - builder.mVertices[a], builder.mVertices[c] - builder.mVertices[a]));
...
void CreateRandomBody(int numPlanes, Vec3 position, float minDist, float maxDist, Vec3 axis, float angle, float invMass,
                      int phase, float stiffness) {
    // 12-kdop
...
NvFlexTriangleMeshId CreateTriangleMesh(Mesh *m) {
    ...
    NvFlexUpdateTriangleMesh(g_flexLib, flexMesh, positions.buffer, indices.buffer, m->GetNumVertices(),
                             m->GetNumFaces(), (float *) &lower, (float *) &upper);
...
void CreateSpringGrid(Vec3 lower, int dx, int dy, int dz, float radius, int phase, float stretchStiffness,
                      float bendStiffness, float shearStiffness, Vec3 velocity, float invMass) {
...
void
CalculateRigidCentersOfMass_ori(const Vec4 *restPositions, int numRestPositions, const int *offsets, Vec3 *translations,
...
// calculates the center of mass of every rigid given a set of particle positions and rigid indices
void
CalculateRigidCentersOfMass(const Vec4 *restPositions, int numRestPositions, const int *offsets, Vec3 *translations,
                            const int *indices, int numRigids) {
    // To improve the accuracy of the result, first transform the restPositions to relative coordinates (by finding the mean and subtracting that from all positions)
    // Note: If this is not done, one might see ghost forces if the mean of the restPositions is far from the origin.
    Vec3 shapeOffset(0.0f);
...
void CalculateRigidLocalPositions(const Vec4 *restPositions, const int *offsets, const Vec3 *translations,
                                  const int *indices, int numRigids, Vec3 *localPositions) {
    int count = 0;
...
void DrawImguiString(int x, int y, Vec3 color, int align, const char *s, ...) {
    ...
    imguiDrawText(x, y, align, buf, imguiRGBA((unsigned char) (color.x * 255), (unsigned char) (color.y * 255),
...
void DrawRect(float x, float y, float w, float h, Vec3 color) {
    imguiDrawRect(x, y, w, h, imguiRGBA((unsigned char) (color.x * 255), (unsigned char) (color.y * 255),
                                        (unsigned char) (color.z * 255)));
}
...
void DrawLine(float x0, float y0, float x1, float y1, float r, Vec3 color) {
    imguiDrawLine(x0, y0, x1, y1, r, imguiRGBA((unsigned char) (color.x * 255), (unsigned char) (color.y * 255),
                                               (unsigned char) (color.z * 255)));
}

void DrawShadowedLine(float x0, float y0, float x1, float y1, float r, Vec3 color) {
    imguiDrawLine(x0 + HELPERS_SHADOW_OFFSET, y0 - HELPERS_SHADOW_OFFSET, x1 + HELPERS_SHADOW_OFFSET,
                  y1 - HELPERS_SHADOW_OFFSET, r, imguiRGBA(0, 0, 0));
    imguiDrawLine(x0, y0, x1, y1, r, imguiRGBA((unsigned char) (color.x * 255), (unsigned char) (color.y * 255),
                                               (unsigned char) (color.z * 255)));
}
...
void SampleMesh(Mesh *mesh, Vec3 lower, Vec3 scale, float rotation, float radius, float volumeSampling,
                float surfaceSampling, std::vector <Vec3> &outPositions) {
    if (!mesh)
        return;
...
void GetShapeBounds(Vec3 &totalLower, Vec3 &totalUpper) {
    ...
    for (int i = 0; i < g_buffers->shapeFlags.size(); ++i) {
        ...
        TransformBounds(localLower, localUpper, Vec3(g_buffers->shapePositions[i]), g_buffers->shapeRotations[i], 1.0f,
