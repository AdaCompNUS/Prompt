...
void pyflex_create_rigid(py::array_t<float> positions) {

    printf("pyflex_create_rigid");
    g_buffers->positions.map();
    g_buffers->rigidIndices.map();
//    g_buffers->velocities.map();
//    g_buffers->phases.map();
//    g_buffers->rigidCoefficients.map();
...
//    g_buffers->rigidOffsets.resize(0);
    ...
    for (size_t i = 0; i < (size_t) positions.size(); i++) {
...
//        g_buffers->rigidLocalNormals.push_back(Vec4(n, d));
        ...
        g_buffers->phases.push_back(NvFlexMakePhase(1, 0));
...
void Init(int scene, py::array_t<float> scene_params, bool centerCamera = true, int thread_idx = 0) {
    ...
    if (centerCamera) {
        ...
        g_camPos = Vec3(0.4999f, 0.8f,0.4999f);
//        g_camPos = Vec3(0.01, 0.7f,0.01);
        g_camAngle = Vec3(DegToRad(0.0f), DegToRad(-80.0001f), DegToRad(0.0f));
...
//    printf("g_params.radius %f \n", g_params.radius);
//    printf("g_params.solidRestDistance %f \n", g_params.solidRestDistance);
//    printf("g_params.dynamicFriction %f \n", g_params.dynamicFriction);
//    printf("g_params.staticFriction %f \n", g_params.staticFriction);
    ...
    if (!g_interop) {
        ...
        if (!headless) {
            if (GetNumDiffuseRenderParticles(g_diffuseRenderBuffers)) {
                NvFlexGetDiffuseParticles(g_solver, g_buffers->diffusePositions.buffer,
...
//    g_device = gpu_id;
    ...
    // Customized scenes
    auto *grasping = new Grasping("Grasping");
    g_scenes.push_back(grasping);

    auto *pushing = new Pushing("Pushing");
    g_scenes.push_back(pushing);
    ...
    SoftBody::Instance s    printf("pyflex_init 1");oftbunny(make_path(bunny_path, "/data/bunny.ply"));
...
//    printf("pyflex_set_positions positions.size() %f \n", (double) positions.size());
...
void pyflex_set_paras(py::array_t<float> paras) {
    auto buf = paras.request();
    auto ptr = (float *) buf.ptr;
    g_params.solidRestDistance = ptr[0];
    g_params.radius = ptr[1];
    NvFlexSetParams(g_solver, &g_params);
}

void pyflex_set_friction(py::array_t<float> paras) {
    auto buf = paras.request();
    auto ptr = (float *) buf.ptr;
    g_params.dynamicFriction = ptr[0];
    NvFlexSetParams(g_solver, &g_params);
}

void pyflex_set_camera_pos(py::array_t<float> paras) {
    auto buf = paras.request();
    auto ptr = (float *) buf.ptr;

//    printf("pyflex_set_camera_pos");

    g_camPos = Vec3(0.0f + ptr[0], 0.4f, 0.6f + ptr[1]);
    g_camAngle = Vec3(DegToRad(0.0f), DegToRad(-20.0001f), DegToRad(0.0f));
    g_scenes[g_scene]->CenterCamera();
    UpdateCamera();
    UpdateEmitters();
//    RenderScene();
...
py::array_t<float> pyflex_get_contacts() {
    const int maxContactsPerParticle = 6;
    ...
    auto contact_lines = py::array_t<float>((size_t) g_buffers->restPositions.size() * 6 * maxContactsPerParticle); // start Vec3 , end Vec3
    auto ptr = (float *) contact_lines.request().ptr;
    int contact_idx = 0;
    ...
    for (int i = 0; i < int(g_buffers->activeIndices.size()); ++i) {
        ...
        for (unsigned int c = 0; c < count; ++c) {
            ...
            Vec3 start = Vec3(g_buffers->positions[g_buffers->activeIndices[i]]);
            Vec3 end = Vec3(g_buffers->positions[g_buffers->activeIndices[i]]) + Vec3(plane) * scale;
            ptr[contact_idx * 6] = start.x;
            ptr[contact_idx * 6 + 1] = start.y;
            ptr[contact_idx * 6 + 2] = start.z;
            ptr[contact_idx * 6 + 3] = end.x;
            ptr[contact_idx * 6 + 4] = end.y;
            ptr[contact_idx * 6 + 5] = end.z;
            contact_idx++;
        }
    }
    ...
    auto contact_lines_resized = py::array_t<float>((size_t) contact_idx * 6); // start Vec3 , end Vec3
    auto ptr_resized = (float *) contact_lines_resized.request().ptr;
    std::copy_n(ptr, contact_idx * 6, ptr_resized);

    return contact_lines_resized;
}


void pyflex_apply_force(int particle_num, py::array_t<float> delta_) {
    auto ptr_delta = (float *) delta_.request().ptr;
    Vec3 delta = Vec3(ptr_delta[0], ptr_delta[1], ptr_delta[2]);

    g_buffers->positions.map();
    g_buffers->velocities.map();
    Vec3 p = Vec3(g_buffers->positions[particle_num]) + delta;

    g_buffers->positions[particle_num].x = p.x;
    g_buffers->positions[particle_num].y = p.y;
    g_buffers->positions[particle_num].z = p.z;

    g_buffers->velocities[particle_num].x = delta.x / g_dt;
    g_buffers->velocities[particle_num].y = delta.y / g_dt;
    g_buffers->velocities[particle_num].z = delta.z / g_dt;

    // g_buffers->velocities[particle_num].x = delta.x * 20.0f;
    // g_buffers->velocities[particle_num].y = delta.y * 20.0f;
    // g_buffers->velocities[particle_num].z = delta.z * 20.0f;

    g_buffers->positions.unmap();
    g_buffers->velocities.unmap();
}

void pyflex_set_com(py::array_t<float> paras) {
    auto buf = paras.request();
    auto ptr = (float *) buf.ptr;
    Vec3 com = Vec3(ptr[0], ptr[1], ptr[2]);
...
//    g_buffers->rigidTranslations.resize(0, Vec3());


//    g_buffers->rigidTranslations[0] = Vec3(ptr[0], ptr[1], ptr[2]);
//    cout << "setting pyflex_set_com to " << ptr[0] << ptr[1] << ptr[2] << "\n";

//    CalculateRigidLocalPositions(&g_buffers->positions[0], &g_buffers->rigidOffsets[0],
//                                 &g_buffers->rigidTranslations[0], &g_buffers->rigidIndices[0], 1,
//                                 &g_buffers->rigidLocalPositions[0]);

    CalculateRigidLocalPositions(&g_buffers->positions[0], &g_buffers->rigidOffsets[0],
                                 &com, &g_buffers->rigidIndices[0], 1,
                                 &g_buffers->rigidLocalPositions[0]);
...
m.def("create_rigid", &pyflex_create_rigid, "Set particle positions");

m.def("set_paras", &pyflex_set_paras, "Set env paras");
m.def("set_friction", &pyflex_set_friction, "Set env friction");
m.def("set_camera", &pyflex_set_camera_pos, "Set camera pos");
m.def("apply_force", &pyflex_apply_force, "apply a force to particle");
m.def("set_com", &pyflex_set_com, "set center of mass or translation");
m.def("get_contacts", &pyflex_get_contacts, "get contacts (N,6) with x0 y0 z0 x1 y1 z1");
